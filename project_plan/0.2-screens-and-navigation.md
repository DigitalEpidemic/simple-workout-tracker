...
**Navigation To**:
- Active Workout (start session and begin timer)
...
**Responsibilities**:
- **Display workout session timer (elapsed time) - STARTS when "Begin Workout" is pressed**
- **Timer runs continuously until workout is completed or cancelled**
- Show exercise list with completion status
- Current exercise highlighted
- Show completed sets with checkmarks
- "Add Exercise" button (mid-workout)
- "Finish Workout" button (stops timer, navigates to summary)
- "Cancel Workout" option (with confirmation, stops timer)
- Display rest timer badge when active (rest timer is separate from workout timer)
- Navigation to each exercise detail

**Navigation From**:
- Start Workout Flow (timer begins on entry)
- Home Screen (resume workout - timer continues from where it was)

**Navigation To**:
- Exercise Detail (tap on exercise)
- Rest Timer Modal (auto-trigger or manual)
- Workout Summary (on completion - timer stopped, total duration calculated)
...
- Display completed workout details:
  - **Total duration (calculated from startTime to endTime when "Finish Workout" was pressed)**
  - Exercises completed
  - Total sets
...
**Navigation From**:
- Active Workout Screen (on finish - workout timer is stopped and endTime is recorded)
