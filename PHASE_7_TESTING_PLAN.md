...
```typescript
// src/lib/db/repositories/__tests__/templates.test.ts
...
1. **Start with Phase 7.1.2** - Test utility functions first (easiest wins)
2. **Then Phase 7.1.3** - Test stores (isolated, no external deps)
3. **Then Phase 7.1.1** - Test repositories (requires database mocking strategy)
